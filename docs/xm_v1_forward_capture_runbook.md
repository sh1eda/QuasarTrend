...
`5ce99bdb5fd64c7d5c043c17170ad7886c77c2752ec8ecd934e460556f99004b`.

From PowerShell in the accepted authorization-freeze checkout:

```powershell
$Repo = "D:\QuasarTrend"
$Evidence = "D:\QuasarTrendEvidence\XMGlobal-MT5-9-bounded-passive-v1-closure-cert7-20260915"
...
python -c "from pathlib import Path; from quasartrend.research.xm_gold_historical_validation import verify_frozen_production_sources; from quasartrend.forward.mt5 import implementation_hash; assert len(verify_frozen_production_sources(Path('.'))) == 22; assert implementation_hash() == '5ce99bdb5fd64c7d5c043c17170ad7886c77c2752ec8ecd934e460556f99004b'; print('SOURCE IDENTITY PASS')"
...
and the failed sixth certificate activation root
`D:\QuasarTrendEvidence\XMGlobal-MT5-9-bounded-passive-v1-closure-cert6-20260915`
are immutable and remain bound to their earlier `implementation_sha256` values;
do not reuse, migrate, edit, or delete any of them. In particular, `closure-cert6`
...
The post-review retry must use the new `closure-cert7` evidence root shown above.
...
evidence, plus the Sep 12 00:00–Sep 14 01:00 UTC weekend closure evidence, and
the Sep 15 00:00–01:00 UTC closure evidence, are specific to
...
Sep 11 23:45 and the Sep 14 01:00 reopen. The Sep 15 M15 certificate excludes
Sep 14 23:45 and the Sep 15 01:00 reopen. These certificates are
...
The Sep 15 certificate uses the user-supplied target-native probe from the
validated XM Global Limited / XMGlobal-MT5 9 / GOLD target. It showed M15 bars
at Sep 14 23:30 and 23:45; no M15 bars at Sep 15 00:00, 00:15, 00:30, or
00:45; and M15 bars at Sep 15 01:00, 01:15, and 01:30. It showed
`M1_INSIDE_CANDIDATE = 0` for the exact candidate interval; the total M1 row
count does not itself prove closure. The last pre-closure tick was
`2026-09-14T23:58:59.290000Z`; the first reopen tick was
`2026-09-15T01:00:02.055000Z`; and the largest observed tick gap was 3662.765
seconds. Only the exact supplied half-open
`[2026-09-15T00:00:00Z, 2026-09-15T01:00:00Z)` M15 interval is admitted.
Previous exact daily certificates are not authority for this interval. This
evidence does not establish any recurring daily, weekday, weekend,
broker-schedule, timezone, UTC/server-time conversion, DST, historical-session,
future, or prospective closure rule. Contradictory tick/M1 activity still
prevents closure admission; late activity contradicting an already committed
closure rejects the observation before state, journal, or checkpoint mutation.
Repository verification uses synthetic fixtures only; this change does not
constitute a new broker probe or authorize a retry before review.
