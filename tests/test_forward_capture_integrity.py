...
SEP10_DAILY_START = 1_788_998_400_000
SEP10_DAILY_END = 1_789_002_000_000
SEP10_TEST_ACTIVATION = 1_789_016_400_000
...
SEP15_DAILY_START = 1_789_430_400_000
SEP15_DAILY_END = 1_789_434_000_000
SEP15_TEST_ACTIVATION = 1_789_448_400_000
...
def test_sep10_restart_and_late_contradiction_preserve_durable_evidence(service_factory, monkeypatch, activity):
    api = SeriesMT5()
    api.server = "XMGlobal-MT5 9"
    shift = SEP10_DAILY_END - START
    for rows in api.all_rates.values():
        for row in rows:
            row.time += shift // 1000
    api.now_ms = SEP10_DAILY_END
    first_m15 = api.all_rates[15][0].time * 1000
    api.all_rates[15][:0] = [rate(first_m15 - offset * M15) for offset in range(4, 0, -1)]
    api.omit[1].update(slots(SEP10_DAILY_START, SEP10_DAILY_END, 60_000))
    api.omit[15].update(slots(SEP10_DAILY_START, SEP10_DAILY_END, M15))
    ...
    if activity == "tick":
        monkeypatch.setattr(resumed, "_tick_rows", lambda now: ([{
            "tick_id": "late-sep10", "time_msc": SEP10_DAILY_START,
            "bid": 100.0, "ask": 100.1,
        }], {}))
    else:
        api.omit[1].remove(SEP10_DAILY_START)
...
@pytest.mark.parametrize("activity", ["tick", "m1"])
def test_sep11_restart_and_late_contradiction_preserve_durable_evidence(service_factory, monkeypatch, activity):
    api = SeriesMT5()
    api.server = "XMGlobal-MT5 9"
    shift = SEP11_DAILY_END - START
    for rows in api.all_rates.values():
        for row in rows:
            row.time += shift // 1000
    api.now_ms = SEP11_DAILY_END
    first_m15 = api.all_rates[15][0].time * 1000
    api.all_rates[15][:0] = [rate(first_m15 - offset * M15) for offset in range(4, 0, -1)]
    api.omit[1].update(slots(SEP11_DAILY_START, SEP11_DAILY_END, 60_000))
    api.omit[15].update(slots(SEP11_DAILY_START, SEP11_DAILY_END, M15))
    ...
    if activity == "tick":
        monkeypatch.setattr(resumed, "_tick_rows", lambda now: ([{
            "tick_id": "late-sep11", "time_msc": SEP11_DAILY_START,
            "bid": 100.0, "ask": 100.1,
        }], {}))
    else:
        api.omit[1].remove(SEP11_DAILY_START)
    with pytest.raises(ValueError, match="activity contradicts committed no-bar certificate"):
        resumed.poll_once()
    assert evidence(resumed) == before_contradiction
    resumed.close()
...
@pytest.mark.parametrize("activity", ["tick", "m1"])
def test_initial_sep11_contradiction_rejects_before_durable_or_state_mutation(
    ...
    if activity == "tick":
        monkeypatch.setattr(service, "_tick_rows", lambda now: ([{
            "tick_id": "initial-sep11", "time_msc": SEP11_DAILY_START,
            "bid": 100.0, "ask": 100.1,
        }], {}))
    else:
        api.omit[1].remove(SEP11_DAILY_START)
    ...
    assert all(journal.count == 0 for journal in service.projections.values())
    assert not service.checkpoint.exists()
    assert {
        path.relative_to(service.root): path.read_bytes()
        for path in service.root.rglob("*") if path.is_file()
    } == before_files
...
@pytest.mark.parametrize("activity", ["tick", "m1"])
def test_sep15_restart_and_late_contradiction_preserve_durable_evidence(
    service_factory, monkeypatch, activity,
):
    api = SeriesMT5()
    api.server = "XMGlobal-MT5 9"
    shift = SEP15_DAILY_END - START
    for rows in api.all_rates.values():
        for row in rows:
            row.time += shift // 1000
    api.now_ms = SEP15_DAILY_END
    first_m15 = api.all_rates[15][0].time * 1000
    api.all_rates[15][:0] = [rate(first_m15 - offset * M15) for offset in range(4, 0, -1)]
    api.omit[1].update(slots(SEP15_DAILY_START, SEP15_DAILY_END, 60_000))
    api.omit[15].update(slots(SEP15_DAILY_START, SEP15_DAILY_END, M15))
    ...
    if activity == "tick":
        monkeypatch.setattr(resumed, "_tick_rows", lambda now: ([{
            "tick_id": "late-sep15", "time_msc": SEP15_DAILY_START,
            "bid": 100.0, "ask": 100.1,
        }], {}))
    else:
        api.omit[1].remove(SEP15_DAILY_START)
    with pytest.raises(ValueError, match="activity contradicts committed no-bar certificate"):
        resumed.poll_once()
    assert evidence(resumed) == before_contradiction
    resumed.close()
...
@pytest.mark.parametrize("activity", ["tick", "m1"])
def test_initial_sep15_contradiction_rejects_before_durable_or_state_mutation(
    service_factory, monkeypatch, activity,
):
    api = SeriesMT5()
    api.server = "XMGlobal-MT5 9"
    ...
    if activity == "tick":
        monkeypatch.setattr(service, "_tick_rows", lambda now: ([{
            "tick_id": "initial-sep15", "time_msc": SEP15_DAILY_START,
            "bid": 100.0, "ask": 100.1,
        }], {}))
    else:
        api.omit[1].remove(SEP15_DAILY_START)
...
def test_exact_sep10_daily_closure_accepts_only_the_certified_m15_slots():
    missing = slots(SEP10_DAILY_START, SEP10_DAILY_END, M15)
    machine = certificate_machine(m15_omissions=missing, activation_ms=SEP10_TEST_ACTIVATION)
    assert machine.initialized and machine.pending_gaps == []
    assert SEP10_DAILY_START - M15 in machine.known["m15"]
    assert SEP10_DAILY_END in machine.known["m15"]


def test_sep10_daily_closure_boundaries_remain_fail_closed():
    at_2345 = SEP10_DAILY_START - M15
    at_reopen = SEP10_DAILY_END
    machine = certificate_machine(
        activation_ms=SEP10_TEST_ACTIVATION,
        m15_omissions=slots(SEP10_DAILY_START, SEP10_DAILY_END, M15)
        | {at_2345, at_reopen},
    )
    assert {gap["open_time"] for gap in machine.pending_gaps} == {
        at_2345, at_reopen,
...
@pytest.mark.parametrize("activity", ["tick", "m1"])
def test_activity_inside_certified_sep10_daily_slot_blocks_admission(activity):
    kwargs = ({"tick_times": (SEP10_DAILY_START,)} if activity == "tick"
              else {"m1_times": (SEP10_DAILY_START,)})
    machine = certificate_machine(
        activation_ms=SEP10_TEST_ACTIVATION,
        m15_omissions=slots(SEP10_DAILY_START, SEP10_DAILY_END, M15), **kwargs,
    )
    assert machine.pending_gaps == [{
        "timeframe": "m15", "open_time": SEP10_DAILY_START,
        "finalized_at": SEP10_DAILY_START + M15,
        "reason": "activity_proven_missing_candle",
    }]


@pytest.mark.parametrize("activity", ["tick", "m1"])
def test_late_sep10_activity_is_rejected_before_capture_state_mutation(activity):
    machine = certificate_machine(
        activation_ms=SEP10_TEST_ACTIVATION,
        m15_omissions=slots(SEP10_DAILY_START, SEP10_DAILY_END, M15),
    )
    ...
    ticks = []
    if activity == "tick":
        ticks.append({"tick_id": "late-sep10", "time_msc": SEP10_DAILY_START,
                      "bid": 100.0, "ask": 100.1})
    else:
        rates["m1"].append({
            "bar_id": f"m1:{SEP10_DAILY_START}", "open_time": SEP10_DAILY_START,
            "finalized_at": SEP10_DAILY_START + DURATIONS["m1"], "timeframe": "m1",
            "open": 100.0, "high": 101.0, "low": 99.0, "close": 100.0,
            "tick_volume": 1, "spread": 10, "real_volume": 1,
        })
    with pytest.raises(ValueError, match="activity contradicts committed no-bar certificate"):
        machine.observe({
            "observation_id": "2", "observed_at": SEP10_TEST_ACTIVATION,
            "cutoff_ms": SEP10_TEST_ACTIVATION,
            "activation_ms": SEP10_TEST_ACTIVATION,
            "ticks": ticks, "rates": rates,
        })
    assert {key: value for key, value in machine.__dict__.items()
...
def test_exact_sep11_daily_closure_accepts_only_the_certified_m15_slots():
    missing = slots(SEP11_DAILY_START, SEP11_DAILY_END, M15)
    machine = certificate_machine(m15_omissions=missing, activation_ms=SEP11_TEST_ACTIVATION)
    assert machine.initialized and machine.pending_gaps == []
    assert SEP11_DAILY_START - M15 in machine.known["m15"]
    assert SEP11_DAILY_END in machine.known["m15"]


def test_sep11_daily_closure_boundaries_remain_fail_closed():
    at_2345 = SEP11_DAILY_START - M15
    at_reopen = SEP11_DAILY_END
    machine = certificate_machine(
        activation_ms=SEP11_TEST_ACTIVATION,
        m15_omissions=slots(SEP11_DAILY_START, SEP11_DAILY_END, M15)
        | {at_2345, at_reopen},
    )
    assert {gap["open_time"] for gap in machine.pending_gaps} == {
        at_2345, at_reopen,
...
@pytest.mark.parametrize("activity", ["tick", "m1"])
def test_activity_inside_certified_sep11_daily_slot_blocks_admission(activity):
    kwargs = ({"tick_times": (SEP11_DAILY_START,)} if activity == "tick"
              else {"m1_times": (SEP11_DAILY_START,)})
    with pytest.raises(ValueError, match="activity contradicts committed no-bar certificate"):
        certificate_machine(
            activation_ms=SEP11_TEST_ACTIVATION,
            m15_omissions=slots(SEP11_DAILY_START, SEP11_DAILY_END, M15), **kwargs,
        )


@pytest.mark.parametrize("activity", ["tick", "m1"])
def test_late_sep11_activity_is_rejected_before_capture_state_mutation(activity):
    machine = certificate_machine(
        activation_ms=SEP11_TEST_ACTIVATION,
        m15_omissions=slots(SEP11_DAILY_START, SEP11_DAILY_END, M15),
    )
    ...
    ticks = []
    if activity == "tick":
        ticks.append({"tick_id": "late-sep11", "time_msc": SEP11_DAILY_START,
                      "bid": 100.0, "ask": 100.1})
    else:
        rates["m1"].append({
            "bar_id": f"m1:{SEP11_DAILY_START}", "open_time": SEP11_DAILY_START,
            "finalized_at": SEP11_DAILY_START + DURATIONS["m1"], "timeframe": "m1",
            "open": 100.0, "high": 101.0, "low": 99.0, "close": 100.0,
            "tick_volume": 1, "spread": 10, "real_volume": 1,
        })
    with pytest.raises(ValueError, match="activity contradicts committed no-bar certificate"):
        machine.observe({
            "observation_id": "2", "observed_at": SEP11_TEST_ACTIVATION,
            "cutoff_ms": SEP11_TEST_ACTIVATION,
            "activation_ms": SEP11_TEST_ACTIVATION,
            "ticks": ticks, "rates": rates,
        })
    assert {key: value for key, value in machine.__dict__.items()
...
def test_exact_sep15_daily_closure_accepts_only_the_certified_m15_slots():
    missing = slots(SEP15_DAILY_START, SEP15_DAILY_END, M15)
    machine = certificate_machine(m15_omissions=missing, activation_ms=SEP15_TEST_ACTIVATION)
    assert machine.initialized and machine.pending_gaps == []
    assert missing == {
        SEP15_DAILY_START,
        SEP15_DAILY_START + M15,
        SEP15_DAILY_START + 2 * M15,
        SEP15_DAILY_START + 3 * M15,
    }
    assert all(stamp not in machine.known["m15"] for stamp in missing)
    assert SEP15_DAILY_START - M15 in machine.known["m15"]
    assert SEP15_DAILY_END in machine.known["m15"]


def test_sep15_daily_closure_boundaries_remain_fail_closed():
    at_2345 = SEP15_DAILY_START - M15
    at_reopen = SEP15_DAILY_END
    machine = certificate_machine(
        activation_ms=SEP15_TEST_ACTIVATION,
        m15_omissions=slots(SEP15_DAILY_START, SEP15_DAILY_END, M15)
        | {at_2345, at_reopen},
    )
    assert {gap["open_time"] for gap in machine.pending_gaps} == {
        at_2345, at_reopen,
...
def test_sep15_daily_closure_does_not_cover_sep16_or_create_a_recurring_rule():
    sep16_start = SEP15_DAILY_START + 24 * 60 * 60_000
    missing = slots(sep16_start, sep16_start + 4 * M15, M15)
    machine = certificate_machine(
        m15_omissions=missing, activation_ms=sep16_start + 5 * 60 * 60_000,
    )
    assert {gap["open_time"] for gap in machine.pending_gaps} == missing
    assert all(gap["reason"] == "unresolved_candle_or_session_closure"
               for gap in machine.pending_gaps)
...
@pytest.mark.parametrize("activity", ["tick", "m1"])
def test_activity_inside_certified_sep15_daily_slot_blocks_admission(activity):
    kwargs = ({"tick_times": (SEP15_DAILY_START,)} if activity == "tick"
              else {"m1_times": (SEP15_DAILY_START,)})
    with pytest.raises(ValueError, match="activity contradicts committed no-bar certificate"):
        certificate_machine(
            activation_ms=SEP15_TEST_ACTIVATION,
            m15_omissions=slots(SEP15_DAILY_START, SEP15_DAILY_END, M15), **kwargs,
        )


@pytest.mark.parametrize("activity", ["tick", "m1"])
def test_late_sep15_activity_is_rejected_before_capture_state_mutation(activity):
    machine = certificate_machine(
        activation_ms=SEP15_TEST_ACTIVATION,
        m15_omissions=slots(SEP15_DAILY_START, SEP15_DAILY_END, M15),
    )
    ...
    ticks = []
    if activity == "tick":
        ticks.append({"tick_id": "late-sep15", "time_msc": SEP15_DAILY_START,
                      "bid": 100.0, "ask": 100.1})
    else:
        rates["m1"].append({
            "bar_id": f"m1:{SEP15_DAILY_START}", "open_time": SEP15_DAILY_START,
            "finalized_at": SEP15_DAILY_START + DURATIONS["m1"], "timeframe": "m1",
            "open": 100.0, "high": 101.0, "low": 99.0, "close": 100.0,
            "tick_volume": 1, "spread": 10, "real_volume": 1,
        })
    with pytest.raises(ValueError, match="activity contradicts committed no-bar certificate"):
        machine.observe({
            "observation_id": "2", "observed_at": SEP15_TEST_ACTIVATION,
            "cutoff_ms": SEP15_TEST_ACTIVATION,
            "activation_ms": SEP15_TEST_ACTIVATION,
            "ticks": ticks, "rates": rates,
        })
    assert {key: value for key, value in machine.__dict__.items()
...
def test_closure_manifest_is_exact_and_frozen_v1_bytes_match_canonical_git():
    assert CERTIFIED_NO_BAR_INTERVALS == (
        ...
        {"certificate_id": "xm9-gold-2026-09-10-daily-closure",
         "source_server": "XMGlobal-MT5 9", "symbol": "GOLD",
         "timeframes": frozenset(("m15",)),
         "start_ms": SEP10_DAILY_START, "end_ms": SEP10_DAILY_END},
        ...
        {"certificate_id": "xm9-gold-2026-09-15-daily-closure",
         "source_server": "XMGlobal-MT5 9", "symbol": "GOLD",
         "timeframes": frozenset(("m15",)),
         "start_ms": SEP15_DAILY_START, "end_ms": SEP15_DAILY_END},
    )
